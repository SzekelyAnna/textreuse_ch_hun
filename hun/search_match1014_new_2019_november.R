...
dir<-paste0(gdir,"passed_clean/ki1014/")
...
dir<-paste0(gdir,"drafts_raw/2010-2014_new/")
...
rel <- data.table(read_excel(dir, sheet = "1014"))
...
# new combo
cut<-str_locate_all(txt$text,"\nindokolás :\n|általános indokolás|\nindokolás\n|i n d o k l á s|i n d o k o l á s|\nindoklás\n|\nindokolás\n|\ni n d o k o l á s\n|\nrészletes indokolás\n|\náltalános indokolás\n|\náltalános indoklás\n|\ná l t a l á n o s|\ni n d o k l á s\n|\ni n d o k o l á s\n|költségvetésének indokolása")
...
# melléklet
cut<-str_locate(as.character(txt6$text),"számú melléklet a.{3,20}törvényhez|melléklet a.{3,20}törvényhez(?!”)") # számú melléklet (!=szöveg)|  számú függelék
...
sfile<-paste0(gdir,"1014txt7.Rda")
saveRDS(txt7, file=sfile)
sfile<-paste0(gdir,"1014txt8.Rda")
