...
dir<-paste0(gdir,"passed_clean/ki9802/")
...
dir<-paste0(gdir,"drafts_raw/1998-2002/")
...
rel <- data.table(read_excel(dir, sheet = "9802"))
...
# new combo
cut<-str_locate_all(txt$text,"indokolás a törv|törvényjavaslat indokolása|általános indokolás|\nindokolás \n|indoklás \n|i n d o k o l á s |i n d o k l á s|i n d o k o l á s| ndokolás\n|\nindoklás\n|\nindokolás\n|\ni n d o k o l á s\n|\nrészletes indokolás\n|\náltalános indokolás\n|\náltalános indoklás\n|\ná l t a l á n o s|\ni n d o k l á s\n|\ni n d o k o l á s\n")
...
#str_extract_all(txt6$text,"(?<=[a-zéáúöüóí]{2,10}[@#]) (?=[tlk] )")
# last steps of cleansing
txt6$text<-str_replace_all(txt6$text,"(?<=[a-zéáúöüóí]{2,10}[@#]) (?=[istlkr] )","")
txt6$text<-str_replace_all(txt6$text," (?=[myl] )","")
#txt6$text<-str_replace_all(txt6$text," [^ae§] "," ")
#txt6$text<-str_replace_all(txt6$text,"[_=\\*\\.\\[\\]„/,\\-§:;~…\\*\\^'\\!\\?\\°\\$\\©\\`\\&\\!!\\<\\>\\«\\‘\\+\\•]","")
...
# melléklet out
cut<-str_locate(as.character(txt6$text),"számú melléklet a.{3,20}törv|melléklet a.{3,20}törv") # számú melléklet (!=szöveg)|
...
sfile<-paste0(gdir,"9802txt7.Rda")
saveRDS(txt7, file=sfile)
sfile<-paste0(gdir,"9802txt8.Rda")
