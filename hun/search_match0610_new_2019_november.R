...
dir<-paste0(gdir,"passed_clean/ki0610/")
...
dir<-paste0(gdir,"drafts_raw/2006-2010_new/")
...
rel <- data.table(read_excel(dir, sheet = "0610"))
...
#df<-data.table(cut[,1])
cut<-str_locate(as.character(txt6$text),"számú melléklet a.{3,20}törv|melléklet a.{3,20}törv|számú függelék ") # számú melléklet (!=szöveg)|
...
sfile<-paste0(gdir,"0610txt7.Rda")
saveRDS(txt7, file=sfile)
sfile<-paste0(gdir,"0610txt8.Rda")
