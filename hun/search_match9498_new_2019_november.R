...
dir<-paste0(gdir,"passed_clean/ki9498/")
...
dir<-paste0(gdir,"drafts_raw/1994-1998m/")
...
#str_extract_all(txt$text,"ô")
txt$text<-str_replace_all(txt$text,"ô","ő")
#str_sub(txt2$text,nchar(txt2$text),nchar(txt2$text))
#unique(txt2$doc_id)
...
rel <- data.table(read_excel(dir, sheet = "9498"))
#unique(rel$tv)
txt$doc_id<-str_replace(txt$doc_id,"(\\.html|\\.txt|\\.pdf)","")
txt[,h:=nchar(doc_id)]
txt[h==3,doc_id:=paste("T","0",stri_sub(doc_id, 2, 3), sep="")]
txt[,h:=nchar(doc_id)]
txt[h==4,doc_id:=paste("T","0",stri_sub(doc_id, 2, 4), sep="")]


cut<-str_locate_all(txt$text,"indokolás a törvényjavaslathoz|törvényjavaslat indokolása|indokolás :\n|általános indokolás :\n|\nn d o k|i n d o k l á s|i n d o k o l á s| ndokolás\n|\nindoklás\n|\nindokolás\n|indokolás\n|\ni n d o k o l á s\n|\nrészletes indokolás\n|\náltalános indokolás\n|\náltalános indoklás\n|\ná l t a l á n o s|\ni n d o k l á s\n|\ni n d o k o l á s\n")
...
#source("/Users/Flora/Documents/MTA/HUN_CH_paper_nov19/Hun_data/all_9018/cleansing1.R")
# § remains!!!!
...
#txt6$text<-str_replace_all(txt6$text,"(?<=[a-zéáúöüóí]{2,10}[@#]) (?=[istlkr] )","")
#txt6$text<-str_replace_all(txt6$text," (?=[myl] )","")
...
sum(is.na(cut)==F)
# end
#cut<-str_locate_all(as.character(txt6$text),str_sub(as.character(txt6$text1),nchar(txt6$text1)-40,nchar(txt6$text1)-1))
...
#df<-data.table(cut[,1])
cut<-str_locate(as.character(txt6$text)," számú melléklet a.{3,30}törv| melléklet a.{3,30}törv| számú függelék ") # számú melléklet (!=szöveg)|
...
fw<-cbind(fw,tvdraft$tv)

# cheking international treaties!!!!
fw[arany1<(-30),]
fw[arany1>(+30),]

# T2849 missing removing row
txt6<-txt6[doc_id!="T2849",]
fw<-fw[doc_id!="T2849",]
...
sfile<-paste0(gdir,"9498txt7.Rda")
saveRDS(txt7, file=sfile)
sfile<-paste0(gdir,"9498txt8.Rda")
